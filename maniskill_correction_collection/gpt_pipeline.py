...
import time
from maniskill_correction_collection.gpt import GPTAgent
...
def parse_args(args=None):
    ...
    parser.add_argument("--correction", action="store_true", help="If true, the correction process will be executed.")
...
def check_target(info, flag):
    if flag == 2:
        return info["is_stacked"]
    else:
        return info["is_grasped"]
...
def main(args):
    ...
    text_embed_correction_name = os.path.join(args.lang_embeds_path, "StackCube-v1-correction.pt")
    ...
    if os.path.exists(text_embed_correction_name):
        text_embed_correction = torch.load(text_embed_correction_name)
    else:
    ...
    gpt_agent = GPTAgent("StackCube-v1")
    ...
    for episode in tqdm.trange(total_episodes):
        ...
        while global_steps < MAX_EPISODE_STEPS and not done:
            ...
            # this_time_embed = text_embed if do_correction_period == 0 else text_embed_correction
            ...
            if args.correction:
                ...
                if condition_check_steps < 0:
                    gpt_info = gpt_agent.request(env.render().squeeze(0).detach().cpu().numpy())
                    reach_target = check_target(gpt_info, condition_flag)
                    if not reach_target:
                        if condition_flag == 2 and gpt_info["is_grasped"]:
                            condition_flag = 2
