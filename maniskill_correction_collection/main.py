...
def save_data_old(save_path, count, obs_image_array, proprio_array, action_array):
    ...
    t0 = time.time()
    with h5py.File(save_path + f'/episode_{count}' + '.hdf5', 'w', rdcc_nbytes=1024**2*2) as root:
        root.attrs['sim'] = True
...
def main(args):
    ...
    if not os.path.exists(correction_success_demo_path):
        os.makedirs(correction_success_demo_path)
    if not os.path.exists(correction_process_path):
        os.makedirs(correction_process_path)
    ...
    for episode in tqdm.trange(total_episodes):
        ...
        while global_steps < MAX_EPISODE_STEPS and not done:
            ...
            if do_ik:
                ...
                if "is_cubeA_grasped" in info and info["is_cubeA_grasped"]:
                    correction_process_count += 1
                    save_data(correction_process_path,
