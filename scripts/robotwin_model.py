...
class RoboticDiffusionTransformerModel(object):
    ...
    def _format_joint_to_state(self, joints):
        ...
        # TODO: The scaling factor should aligne with Robottwin env.
        # joints = joints / torch.tensor(
        #     [[[1, 1, 1, 1, 1, 1, 4.7908, 1, 1, 1, 1, 1, 1, 4.7888]]],
        #     device=joints.device, dtype=joints.dtype
        # )
        joints[:,:, [6, 13]] = (joints[:, :, [6, 13]] > 0) * (joints[:, :, [6, 13]]) / torch.tensor([[[0.045, 0.045]]], device=joints.device, dtype=joints.dtype) + (joints[:, :, [6, 13]] <= 0) * (joints[:, :, [6, 13]]) 
        # joints = joints / torch.tensor(
